...
ModuleMetadata* package_load_module_metadata(const char* path) {
    ...
    if (path_len > 12 && strcmp(path + path_len - 12, ".swiftmodule") == 0) {
        ...
        if (exports_start) {
            ...
            const char* array_start = strchr(exports_start, '[');
            if (array_start) {
                // Find array end
                const char* array_end = array_start + 1;
                int bracket_count = 1;
                while (*array_end && bracket_count > 0) {
                    if (*array_end == '[') bracket_count++;
                    else if (*array_end == ']') bracket_count--;
                    array_end++;
                }
                
                // Extract array
                size_t len = array_end - array_start;
                char* exports_array = malloc(len + 1);
                strncpy(exports_array, array_start, len);
                exports_array[len] = '\0';
                
                // Parse the array of export names
                char** export_names = json_parse_string_array(exports_array, &metadata->export_count);
                free(exports_array);
                
                // Convert to ModuleExport structures
                if (export_names && metadata->export_count > 0) {
                    metadata->exports = calloc(metadata->export_count, sizeof(ModuleExport));
                    for (size_t i = 0; i < metadata->export_count; i++) {
                        metadata->exports[i].name = export_names[i];
                        metadata->exports[i].type = MODULE_EXPORT_FUNCTION; // Default type
                        // Other fields will be filled when the module is loaded
                    }
                    free(export_names); // Free the array but not the strings (they're now owned by exports)
                } else {
    ...
    if (exports_start) {
        // fprintf(stderr, "Found exports section\n");
        ...
        if (exports_start) {
            ...
            int brace_count = 1;
            int nested_brace_count = 0;
            bool in_string = false;
            
            // fprintf(stderr, "Counting exports in: %.100s...\n", exports_start);
            
            while (*p && brace_count > 0) {
                if (!in_string) {
                    if (*p == '"' && nested_brace_count == 0) {
                        in_string = true;
                        // This might be an export name - only count at top level
                        const char* name_end = strchr(p + 1, '"');
                        if (name_end) {
                            // Skip whitespace after closing quote
                            const char* check = name_end + 1;
                            while (*check && (*check == ' ' || *check == '\t' || *check == '\n' || *check == '\r')) {
                                check++;
                            }
                            if (*check == ':') {
                                metadata->export_count++;
                                // fprintf(stderr, "Found export, count now: %zu\n", metadata->export_count);
                            }
                        }
                    } else if (*p == '{') {
                        brace_count++;
                        if (brace_count > 1) nested_brace_count++;
                    } else if (*p == '}') {
                        brace_count--;
                        if (nested_brace_count > 0) nested_brace_count--;
                    }
                } else if (*p == '"' && *(p - 1) != '\\') {
                    in_string = false;
                }
            ...
            brace_count = 1;
            while (*p && export_idx < metadata->export_count && brace_count > 0) {
                if (*p == '"' && nested_brace_count == 0) {
                    ...
                    if (name_end) {
                        ...
                        if (*check == ':') {
                            ...
                            // Find export object
                            const char* obj_start = strchr(check, '{');
                            if (obj_start) {
                            ...
                            if (path_end) {
                                ...
                                if (strncmp(dep_path, "file:", 5) == 0) {
                                    ...
                                    if (*file_path == '/' && *(file_path + 1) == '/') {
                                        // Skip extra slashes after file:
                                        while (*file_path == '/' && *(file_path + 1) == '/') file_path++;
                                    }
...
bool package_resolve_dependencies(PackageSystem* pkg_sys, ModuleMetadata* module) {
    if (!pkg_sys || !module) return false;
    
    // Check each dependency
    for (size_t i = 0; i < module->dependency_count; i++) {
        const char* dep_name = module->dependencies[i].name;
        const char* dep_version = module->dependencies[i].version;
        
        // Look for dependency in the package system
        bool found = false;
        for (size_t j = 0; j < pkg_sys->dependencies.count; j++) {
            if (strcmp(pkg_sys->dependencies.names[j], dep_name) == 0) {
                found = true;
                break;
            }
        }
        ...
        if (!found) {
            for (size_t j = 0; j < pkg_sys->cache.count; j++) {
                if (strcmp(pkg_sys->cache.modules[j]->name, dep_name) == 0) {
                    // Check version compatibility
                    const char* cached_version = pkg_sys->cache.modules[j]->version;
                    
                    // If no version requirement, any version is OK
                    if (!dep_version || strlen(dep_version) == 0) {
                        found = true;
                        break;
                    }
                    
                    // If cached module has no version, can't verify
                    if (!cached_version) {
                        fprintf(stderr, "Warning: Module '%s' has no version info, assuming compatible\n", dep_name);
                        found = true;
                        break;
                    }
        ...
        if (!found) {
            fprintf(stderr, "Unresolved dependency: %s@%s required by %s\n", 
                    dep_name, dep_version ? dep_version : "any", module->name);
            return false;
        }
    }
