...
static void resize(HashMap* map) {
    ...
    HashMapEntry** new_buckets = MEM_NEW_ARRAY(map->allocator, HashMapEntry*, new_capacity);
    if (!new_buckets) return;  // Failed to resize, keep current size
    ...
    MEM_FREE(map->allocator, map->buckets, sizeof(HashMapEntry*) * map->capacity);
